...
| 会议纪要生成器 | 会议主题、日期、参会人和原始记录整理 | 已完成 | 输出背景、结论、行动项、风险和待确认问题，并支持复制 Markdown |
...
| 项目 | 当前状态 | 说明 |
| --- | --- | --- |
| Next.js App Router | 已完成 | 25 个静态/SSG 页面可生产构建 |
...
| 会议纪要模板生成 | 已完成 | 不接外部模型，负责人或截止时间缺失时标注待确认 |
| 深色模式 | 已完成 | 支持本地保存 |
...
已在本地开发服务 `http://localhost:3000` 完成会议纪要生成器检查：

| 检查项 | 检查结果 |
| --- | --- |
| `/meeting-minutes` 页面标题、表单和默认原始记录可见 | 通过 |
| Markdown 输出包含行动项表格 | 通过 |
| 缺失负责人或截止时间时出现“待确认” | 通过 |
| 默认示例中的“周五前”截止时间可识别 | 通过 |
| 桌面端点击“复制 Markdown”后显示“已复制” | 通过 |
| 移动端 375x812 标题、表单、Markdown 表格和“待确认”可见 | 通过 |
| 移动端 375x812 无横向溢出 | 通过 |

说明：当前工具使用本地模板逻辑，不接外部模型；复杂会议记录仍需人工复核。
