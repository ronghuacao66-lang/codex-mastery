# 视频精选

> 本文件由 `npm run export:content` 从 `data/videos.json` 生成。JSON 是页面渲染的数据源，Markdown 是可读备份。

共 12 条内容。

## OpenAI官方：Codex新手教程

**平台:** Bilibili

**创作者:** OpenAI官方

**链接:** https://www.bilibili.com/video/BV19r39z4EMy/

**时长:** 约 8 分钟

**阶段:** 入门

**主题:** Codex 入门

**摘要:** 适合完全没用过 Codex 的用户，快速理解 Codex 的基本使用方式和任务入口。

**推荐理由:** 作为第一支视频，帮助中国用户用中文环境建立 Codex 的正确第一印象。

**学习收获:**

- 理解 Codex 能做什么
- 知道如何开始一个任务
- 形成第一次任务委托思路

**笔记模板:**

我看到的 3 个 Codex 使用要点：
1. 
2. 
3. 
我准备交给 Codex 的第一个小任务：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- Codex
- 新手
- 官方
- 入门

## 40分钟学会 Codex：OpenAI 最强编程助手实操教程

**平台:** Bilibili

**创作者:** B站创作者

**链接:** https://www.bilibili.com/video/BV1Nd3CzFEGD/

**时长:** 约 40 分钟

**阶段:** 入门

**主题:** 完整实操

**摘要:** 从使用场景到实操流程的长视频，适合配合 30 天训练营前 3 天观看。

**推荐理由:** 时长适中，适合一次性建立 Codex 使用全局观，不只是看零散功能。

**学习收获:**

- 理解 Codex 任务流程
- 知道如何描述需求
- 能跟做一个基础工程任务

**笔记模板:**

这支视频中我最想复用的任务写法：

我还没理解的地方：

我会用 Codex 复刻的练习：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- Codex
- 实操
- 长课
- 中文

## OpenAI 全新 Codex CLI 命令行工具实操演示

**平台:** Bilibili

**创作者:** B站创作者

**链接:** https://www.bilibili.com/video/BV15yd8YNEqF/

**时长:** 约 16 分钟

**阶段:** 进阶

**主题:** Codex CLI

**摘要:** 聚焦 Codex CLI 的命令行使用方式，适合想从网页体验进入本地工程流的用户。

**推荐理由:** 掌握 CLI 思维后，用户更容易理解仓库、命令、验证和自动化的关系。

**学习收获:**

- 理解 CLI 与网页对话的差异
- 知道如何把任务交给本地工程环境
- 认识命令行验证的重要性

**笔记模板:**

视频中的 CLI 工作流：

我当前项目可以运行的命令：

我会写进 AGENTS.md 的规则：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- Codex CLI
- 命令行
- 本地开发
- 进阶

## Codex CLI：如何用 Codex 生成与修改代码

**平台:** Bilibili

**创作者:** B站创作者

**链接:** https://www.bilibili.com/video/BV1HPcLe5Eqe/

**时长:** 约 12 分钟

**阶段:** 进阶

**主题:** 代码生成与修改

**摘要:** 适合已经完成基础入门后，学习如何让 Codex 生成代码、修改代码并理解改动。

**推荐理由:** 能帮助用户从“让 AI 写代码”升级到“让 Codex 在项目里完成工程任务”。

**学习收获:**

- 写出更具体的代码修改任务
- 理解修改范围控制
- 学习如何审查 Codex 输出

**笔记模板:**

我学到的代码修改 Prompt：

应该限制的修改范围：

交付前要检查的内容：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- 代码生成
- 代码修改
- CLI
- 工程实践

## Codex 核心三部曲：项目级 AGENTS.md 实战指南

**平台:** Bilibili

**创作者:** B站创作者

**链接:** https://www.bilibili.com/video/BV1TQ3pztE9h/

**时长:** 约 14 分钟

**阶段:** 进阶

**主题:** AGENTS.md

**摘要:** 讲解如何用 AGENTS.md 给 Codex 提供项目级规则，适合训练营第 26 天前后学习。

**推荐理由:** AGENTS.md 是把一次性经验沉淀成长期工程规则的关键能力。

**学习收获:**

- 理解 AGENTS.md 的作用
- 知道哪些规则适合长期保存
- 能为项目写第一版 AGENTS.md

**笔记模板:**

我项目里的稳定规则：
- 技术栈：
- 数据目录：
- 验证命令：
- 禁止事项：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- AGENTS.md
- 项目规则
- 上下文
- 团队协作

## Codex 3.0 更新：Chrome 扩展、Slack 集成与代码审查

**平台:** Bilibili

**创作者:** AICodeKing

**链接:** https://www.bilibili.com/video/BV1df3PzKEHP/

**时长:** 约 17 分钟

**阶段:** 高阶

**主题:** Codex 新能力

**摘要:** 介绍 Codex 3.0 相关更新和工作流能力，适合已经熟悉基础任务后了解进阶用法。

**推荐理由:** 帮助用户理解 Codex 不只是写代码，还能进入代码审查、协作和浏览器工作流。

**学习收获:**

- 了解 Codex 新交互入口
- 理解代码审查型任务
- 思考团队协作工作流

**笔记模板:**

我会尝试的新工作流：

适合团队使用的场景：

需要注意的边界：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 命令行 HTTP 检查可访问。


**标签:**

- Codex 3.0
- Chrome
- Slack
- Code Review

## Getting Started with Codex

**平台:** YouTube

**创作者:** OpenAI

**链接:** https://www.youtube.com/watch?v=px7XlbYgk7I

**时长:** 约 10 分钟

**阶段:** 入门

**主题:** 官方入门

**摘要:** OpenAI 官方 Codex 入门视频，适合配合 Academy 第 1 章学习。

**推荐理由:** 官方视角能帮助用户确认 Codex 的产品定位和基础使用路径。

**学习收获:**

- 建立官方概念框架
- 理解 Codex 基本任务模式
- 知道如何开始第一次使用

**笔记模板:**

官方视频中的关键概念：

我会立刻尝试的任务：

我需要补充的上下文：


**linkStatus:**

**status:** network_or_timeout

**checkedAt:** 2026-06-09

**httpCode:** 000

**note:** 当前本机命令行到 YouTube 超时，需用浏览器或可访问 YouTube 的网络复核。


**标签:**

- OpenAI
- 官方
- YouTube
- 入门

## OpenAI Codex Essentials - AI Coding Agent Course

**平台:** YouTube

**创作者:** freeCodeCamp / ExamPro

**链接:** https://www.youtube.com/watch?v=u-Jl7bzab8A

**时长:** 约 6 小时 36 分钟

**阶段:** 高阶

**主题:** 系统课程

**摘要:** 系统化 Codex 长课，适合周末集中学习或作为训练营补充材料。

**推荐理由:** 内容完整，适合希望系统掌握 Codex、coding agent 和真实工程流的用户。

**学习收获:**

- 系统学习 Codex 工作方式
- 建立 coding agent 认知
- 补齐大型任务执行方法

**笔记模板:**

我准备分几次看完：

每次学习后的实操任务：

最终要完成的项目：


**linkStatus:**

**status:** network_or_timeout

**checkedAt:** 2026-06-09

**httpCode:** 000

**note:** 当前本机命令行到 YouTube 超时，需用浏览器或可访问 YouTube 的网络复核。


**标签:**

- freeCodeCamp
- 长课
- AI Coding Agent
- 系统学习

## Codex 3.0: New Upgrades to OpenAI's AI Coding Agent

**平台:** YouTube

**创作者:** AICodeKing

**链接:** https://www.youtube.com/watch?v=hQkmShTSdN8

**时长:** 约 17 分钟

**阶段:** 高阶

**主题:** 新能力与工作流

**摘要:** 围绕 Codex 3.0 新能力讲解，包括浏览器扩展、协作和代码审查相关工作流。

**推荐理由:** 适合已经会用 Codex 的用户，了解如何把 Codex 接入更完整的研发协作。

**学习收获:**

- 了解 Codex 新能力
- 形成代码审查任务思路
- 评估团队协作场景

**笔记模板:**

新能力清单：

适合我团队的能力：

不适合立即使用的原因：


**linkStatus:**

**status:** network_or_timeout

**checkedAt:** 2026-06-09

**httpCode:** 000

**note:** 当前本机命令行到 YouTube 超时，需用浏览器或可访问 YouTube 的网络复核。


**标签:**

- Codex 3.0
- YouTube
- 协作
- 代码审查

## AI 也能帮你剪视频了：Codex + HyperFrames 实测

**平台:** 抖音精选

**创作者:** 抖音创作者

**链接:** https://www.douyin.com/jingxuan/m/video/7640106137891454244

**时长:** 短视频

**阶段:** 进阶

**主题:** Codex 工具化场景

**摘要:** 短视频形式展示 Codex 与视频工具结合的可能性，适合启发自动化工具开发思路。

**推荐理由:** 帮助用户跳出纯代码视角，理解 Codex 可以参与多媒体和工具链自动化。

**学习收获:**

- 理解 Codex 的工具化想象空间
- 思考跨工具工作流
- 找到一个自动化办公练习方向

**笔记模板:**

这个短视频启发我的自动化场景：

输入是什么：
输出是什么：
可以让 Codex 开发的工具：


**linkStatus:**

**status:** likely_broken

**checkedAt:** 2026-06-09

**httpCode:** 404

**note:** 命令行 HTTP 检查返回 404，未找到可信替代链接，保留供人工复核。


**标签:**

- 抖音精选
- Codex
- 视频工具
- 自动化

## ChatGPT 公式：Prompt 基础短视频

**平台:** 抖音精选

**创作者:** 抖音创作者

**链接:** https://www.douyin.com/jingxuan/m/video/7354683030972140839

**时长:** 短视频

**阶段:** 入门

**主题:** Prompt 基础

**摘要:** 适合零基础用户快速理解 Prompt 不是一句话提问，而是包含角色、任务、上下文和输出要求。

**推荐理由:** 虽然不是 Codex 专属，但能帮助中国用户建立 Prompt 结构意识，再迁移到 Codex 任务委托。

**学习收获:**

- 理解 Prompt 基本结构
- 知道上下文的重要性
- 能迁移到 GCCD 写法

**笔记模板:**

我会迁移到 Codex 的 Prompt 结构：
角色：
任务：
上下文：
输出：


**linkStatus:**

**status:** likely_broken

**checkedAt:** 2026-06-09

**httpCode:** 404

**note:** 命令行 HTTP 检查返回 404，未找到可信替代链接，保留供人工复核。


**标签:**

- 抖音精选
- Prompt
- 入门
- 结构化

## Introduction to Codex

**平台:** OpenAI Academy

**创作者:** OpenAI Academy

**链接:** https://academy.openai.com/en/public/videos/introduction-to-codex-2026-03-02

**时长:** 官方课程视频

**阶段:** 入门

**主题:** 官方课程

**摘要:** OpenAI Academy 的 Codex 介绍视频，适合想从官方课程体系学习的用户。

**推荐理由:** 比碎片视频更适合作为学习路径起点，能帮助用户理解 Codex 在 OpenAI 体系中的定位。

**学习收获:**

- 理解官方课程脉络
- 建立 Codex 基础概念
- 准备后续实操任务

**笔记模板:**

官方课程里的 3 个关键词：
1. 
2. 
3. 
我下一步要完成的实操：


**linkStatus:**

**status:** ok

**checkedAt:** 2026-06-09

**httpCode:** 200

**note:** 旧链接返回 404，已替换为当前可访问的 OpenAI Academy 官方链接。


**标签:**

- OpenAI Academy
- 官方
- 课程
- 入门

